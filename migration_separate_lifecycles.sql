...
BEGIN 
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='rdv' AND column_name='status') THEN
    ...
    SET status = 
      CASE 
        WHEN status IN ('arrive', 'en_attente', 'en_consultation', 'a_encaisser') THEN 'confirme'
    ...
    SET arrival_status = 
      CASE 
        WHEN status IN ('arrive', 'en_attente') THEN 'WAITING'
        WHEN status = 'en_consultation' THEN 'IN_CONSULTATION'
        WHEN status IN ('a_encaisser', 'termine') THEN 'LEFT'
        ELSE 'NOT_ARRIVED'
      END;
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='rdv' AND column_name='statut') THEN
    ...
    SET statut = 
      CASE 
        WHEN statut IN ('arrive', 'en_attente', 'en_consultation', 'a_encaisser') THEN 'confirme'
    ...
    SET arrival_status = 
      CASE 
        WHEN statut IN ('arrive', 'en_attente') THEN 'WAITING'
        WHEN statut = 'en_consultation' THEN 'IN_CONSULTATION'
        WHEN statut IN ('a_encaisser', 'termine') THEN 'LEFT'
        ELSE 'NOT_ARRIVED'
      END;
  END IF;
END $$;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='rdv' AND column_name='status') THEN
    ...
    END IF;
    UPDATE public.rdv SET status = 'confirme', confirmed_at = NOW(), confirmed_by = auth.uid(), confirmation_method = p_method WHERE id = p_rdv_id RETURNING * INTO rdv_after;
  ELSE
    UPDATE public.rdv SET statut = 'confirme', confirmed_at = NOW(), confirmed_by = auth.uid(), confirmation_method = p_method WHERE id = p_rdv_id RETURNING * INTO rdv_after;
  END IF;
